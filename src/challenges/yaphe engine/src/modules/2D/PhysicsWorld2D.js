...
class PhysicsWorld2D {
    constructor(parent) {
        ...
        this.engine = parent;
        this.bindParticles = true;
        this.mousePos = {
            start: new Vector2D(),
            position: new Vector2D(),
            end: new Vector2D(),
            releasePoint: new Vector2D()
        }
    ...
    createCanvas(width = null, height = null) {
        ...
        this.engine.rootElement.appendChild(this.canvas);
    ...
    throwParticle(x, y) {
        let releasePoint = new Vector2D(x, y);
        let distanceTravelled = releasePoint.distanceTo(this.mousePos.start);

        if (distanceTravelled == 0) {
            releasePoint = this.mousePos.releasePoint.copy();
            distanceTravelled = releasePoint.distanceTo(this.mousePos.start) * -1;
        };

        const force = releasePoint.subtract(this.mousePos.start);
        force.normalize();
        force.scalarMultiply(distanceTravelled * 0.1);
        this.particleClosestToMouse.applyForce(force);
    }
    initializeInputHandler() {
        ...
        this.canvas.onmouseup = (e) => {
            if (this.particleClosestToMouse != null) {                
                this.throwParticle(e.clientX, e.clientY);
    ...
    getParticleCloseToMouse() {
        ...
        this.particles.forEach((particle) => {
            const distanceToParticle = particle.position.distanceTo(this.mousePos.position);
    ...
    moveParticle(x, y) {
        if (this.isMovingParticle && this.particleClosestToMouse !== null) {
            const currentPoint = new Vector2D(x, y);

            if (currentPoint.distanceTo(this.mousePos.start) > 20) {
                this.mousePos.releasePoint = this.mousePos.start;
                this.mousePos.start = currentPoint;
            }
    ...
    createSpring2D(a = null, b = null, stiffness = 0.1) {
    ...
    createRope(x = this.center.x, y = this.center.y - 100, length = 200, segments = 10) {
        const rope = new Rope2D(this.pen, x, y, length, segments);
    ...
    handleParticles() {
        this.particles.forEach((particle) => {
            ...
            if (this.bindParticles) {
                    if (particle.isOutOfBoundary(this.boundary)) {
                    particle.constrain(this.boundary);
                    
                    if (particle.isOnCeiling(this.boundary) && this.applyReactionForce) {
                        particle.velocity.y *= -0.9;
                    }

                    if (particle.isOnWall(this.boundary) && this.applyReactionForce) {
                        particle.velocity.x *= -1;
                    }

                    if (particle.isOnFloor(this.boundary) && this.applyReactionForce) {
                        particle.velocity.y = Math.abs(particle.velocity.y) * -0.99;
                    }
                }
            }
